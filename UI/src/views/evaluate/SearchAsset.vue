    ...
    <div class="div-container">
      <div class="content-grid grid" oncontextmenu="return false;">
        <table class="table-asset" id="tableAsset">
          ...
          <thead>
            <tr>
              <th rowspan="2" style="text-align: left">STT</th>
              <th
                sortProp="code"
                sortOrder="asc"
                id="columnAssetCode"
            ...
            <tr>
              <th>
                <div class="header-input">
                  <input v-model="filterCode" maxlength="255" type="text" />
                  <div class="icon icon-filter"></div>
                </div>
              </th>
              <th>
                <div class="header-input">
                  <input v-model="filterName" maxlength="255" type="text" />
                  <div class="icon icon-filter"></div>
                </div>
              </th>
              <th>
                <div class="header-input">
                  <input v-model="filterType" maxlength="255" type="text" />
                  <div class="icon icon-filter"></div>
                </div>
              </th>
              <th>
              ...
              <th>
                <div class="header-input">
                  <input v-model="filterPrice" maxlength="255" type="text" />
                  <div class="icon icon-filter"></div>
                </div>
              </th>
            </tr>
...
export default {
  ...
  computed: {
    filteredList() {
      var self = this;
      return this.listAsset.filter((x) => {
        return (
          self
            .removeAccents(x.assetCode)
            .indexOf(self.removeAccents(self.filterCode)) != -1 &&
          self
            .removeAccents(x.assetName)
            .indexOf(self.removeAccents(self.filterName)) != -1 &&
          self
            .removeAccents(x.assetTypeName)
            .indexOf(self.removeAccents(self.filterType)) != -1 &&
          self
            .removeAccents(x.departmentName)
            .indexOf(self.removeAccents(self.filterDepartment)) != -1 &&
          (!parseInt(self.filterPrice) ||
            parseInt(x.originalPrice) > parseInt(self.filterPrice))
        );
      });
    },
  },
      ...
      await axios
        ...
        .then((response) => {
          ...
          for (let index = 0; index < response.data.length; index++) {
            ...
            res.listAsset.push({
              assetId: e.idtaisan,
              assetCode: e.mataisan,
              assetName: e.tentaisan,
              departmentName: e.bophansudung,
              assetTypeName: e.tenloaitaisan,
              originalPrice: e.nguyengia,
            });
            res.amountAsset++; // đếm tổng số bản ghi
            if (e.nguyengia != null) {
              res.totalPrice += parseInt(e.nguyengia); // tính tổng nguyên giá
            }
          }
