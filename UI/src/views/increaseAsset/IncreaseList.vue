        ...
        <table class="table-increase table-asset" id="tableAsset">
          ...
          <tbody>
            ...
            >
              ...
              <td>{{ index + 1 }}</td>
              <td
                :title="item.assetIncreaseId"
                @click.stop="showDialog('update', item.assetIncreaseId)"
              >
                {{ item.exhibitCode }}
              </td>
              <td
                :title="item.exhibitDate | formatDate(item.exhibitDate)"
                style="text-align: center"
              >
                {{ item.exhibitDate | formatDate(item.exhibitDate) }}
              </td>
              <td
                :title="item.increaseDate | formatDate(item.increaseDate)"
                style="text-align: center"
              >
                {{ item.increaseDate | formatDate(item.increaseDate) }}
              </td>
...
export default {
  ...
  methods: {
    ...
    resizeDetail(event) {
      ...
      var containerDetail = document.getElementsByClassName(
        "grid-detail-container"
      )[0];
      ...
      function mousemove(event) {
        if (!this.resizing) {
          ...
          var condition =
            percent3 +
            parseFloat(((prevY - event.clientY) / rect4.height) * 100);
          if (condition > 27 && condition < 81) {
            containerDetail.style.height =
              "calc(" +
              percent3.toFixed() +
              "% + " +
              (prevY - event.clientY).toFixed() +
              "px)";
            tableAssetIncrease.style.height =
              "calc(" +
              percent2.toFixed() +
              "% - " +
              (prevY - event.clientY).toFixed() +
